...
module.exports = (Users, Products) => {
  ...
  async function crawlProductsInDB() {
  ...
  async function loadDatabaseIntoMemory () { //get all products crwaled
      return new Promise((resolve, reject) => {
        Products.loadDatabase((err) => {
            if(!err) {
                resolve("Loaded Databse into memory"); // database was loaded into memory
            } else { // display custom error message on reject
                reject({error: err, resMessage: `Error loading products database into memory updateHandler.js line '70', rejected at line 74: actual error message: ${err}`});
            }
        });
      })
  }
